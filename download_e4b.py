...
gguf_path = os.path.join(target_dir, "google_gemma-4-E4B-it-Q4_K_M.gguf")
modelfile_path = os.path.join(target_dir, "Modelfile_e4b")
...
def create_modelfile():
    log("Creating Modelfile for E4B...")
    modelfile_content = f"""FROM ./google_gemma-4-E4B-it-Q4_K_M.gguf
...
def register_ollama_model():
    ...
    try:
        cmd = ["ollama", "create", "gemma4:e4b-custom", "-f", modelfile_path]
...
def main():
    ...
    if success:
        log("All steps completed successfully! Gemma 4 E4B Custom model is ready.")
    else:
        sys.exit(1)
