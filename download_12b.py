...
gguf_path = os.path.join(target_dir, "gemma-4-12B-it-Q4_K_M.gguf")
modelfile_path = os.path.join(target_dir, "Modelfile_12b")

url = "https://huggingface.co/bartowski/gemma-4-12B-it-GGUF/resolve/main/gemma-4-12B-it-Q4_K_M.gguf?download=true"
...
def create_modelfile():
    log("Creating Modelfile for 12B...")
    modelfile_content = f"""FROM ./gemma-4-12B-it-Q4_K_M.gguf
...
def register_ollama_model():
    ...
    try:
        cmd = ["ollama", "create", "gemma4:12b-custom-q4", "-f", modelfile_path]
...
def main():
    ...
    if success:
        log("All steps completed successfully! Gemma 4 12B Custom Q4 model is ready.")
    else:
        sys.exit(1)
