// AXIOS CLI Desktop 렌더러 — 익스텐션 디자인 그대로. preload window.connect 로 통신.
...
function applyCfgLabels() {
  const ver = cfg.appVersion ? ` (v${cfg.appVersion})` : '';
  $('brandSuffix').textContent = cfg.company ? `· ${cfg.company}${ver}` : ver;
...
function stopSpriteLoop() {
  if (voRaf) {
    cancelAnimationFrame(voRaf);
    voRaf = 0;
  }
}
function stopOfficeLife() {
  if (lifeTimer) {
...
function pwStopLoop() {
  ...
  if ((window as any)._pwWander) {
    clearInterval((window as any)._pwWander);
    (window as any)._pwWander = null;
  }
}
...
async function loadAiPanel() { renderOfficePreview(); renderTeamRoster(); renderAutomations(); await Promise.all([renderAiCurrent(), loadLocalAI(), loadParams()]); }

let automationsInitialized = false;
async function renderAutomations() {
  const container = $('autoListContainer');
  const agentSelect = $('autoAgent') as HTMLSelectElement;
  if (!container || !agentSelect) return;

  // Render agent options
  agentSelect.innerHTML = AGENT_ORDER.map(id => {
    const a = AGENTS[id];
    return `<option value="${id}">${a.emoji} ${agName(id)} (${a.role})</option>`;
  }).join('');

  // Render modal agent options too
  const modalAgentSelect = $('modalAutoAgent') as HTMLSelectElement;
  if (modalAgentSelect) {
    modalAgentSelect.innerHTML = AGENT_ORDER.map(id => {
      const a = AGENTS[id];
      return `<option value="${id}">${a.emoji} ${agName(id)} (${a.role})</option>`;
    }).join('');
  }
  ...
  if (!automationsInitialized) {
    ...
    $('autoAddToggleBtn')?.addEventListener('click', () => {
      openOverlay('autoSchedulerModal');
    });

    $('modalAutoCancelBtn')?.addEventListener('click', () => {
      closeOverlay('autoSchedulerModal');
      ($('modalAutoTask') as HTMLTextAreaElement).value = '';
    });

    $('modalAutoRegisterBtn')?.addEventListener('click', async () => {
      const agentId = modalAgentSelect.value;
      const taskVal = ($('modalAutoTask') as HTMLTextAreaElement).value.trim();
      const interval = Number(($('modalAutoInterval') as HTMLSelectElement).value);

      if (!taskVal) {
        hint('수행할 할 일을 입력해 주세요.');
        return;
      }

      await connect.automationRegister(agentId, taskVal, interval);
      closeOverlay('autoSchedulerModal');
      ($('modalAutoTask') as HTMLTextAreaElement).value = '';
      hint('🔄 자동화 스케줄이 등록되었습니다.');
      renderAutomations();
    });

    $('autoCancelBtn')?.addEventListener('click', () => {
      $('autoFormPanel').classList.add('hidden');
      ($('autoTask') as HTMLTextAreaElement).value = '';
    });

    $('autoRegisterBtn')?.addEventListener('click', async () => {
      const agentId = agentSelect.value;
      const taskVal = ($('autoTask') as HTMLTextAreaElement).value.trim();
      const interval = Number(($('autoInterval') as HTMLSelectElement).value);
  ...
  // Load and draw the active automations list
  ensureAutomationsStream().catch(() => {});
  ensureSystemStatusStream().catch(() => {});
  let list = await connect.automationList() || [];
  const fbList = Object.values(automationsMap).filter(Boolean);
  if (fbList.length > 0) {
    list = fbList;
  }
  
  const listHtml = list.length === 0
    ? `<div class="muted small" style="text-align:center; padding:12px; border:1px dashed #1f3521; border-radius:6px; width:100%; box-sizing:border-box;">현재 활성화된 자동화 작업이 없습니다.</div>`
    : list.map((item: any) => {
      const agent = AGENTS[item.agentId] || { name: item.agentId, emoji: '🤖', color: '#39ff14' };
      const name = agName(item.agentId);
      return `
        <div class="auto-item" style="display:flex; justify-content:space-between; align-items:center; background:#0c130d; border:1px solid #1f3521; border-radius:6px; padding:8px 12px; gap:8px;">
          <div style="display:flex; align-items:center; gap:8px; flex:1; min-width:0;">
            <div class="tr-av tr-av-emoji" style="border-color:${agent.color}; background:color-mix(in srgb, ${agent.color} 22%, #0a120c); font-size:14px; width:28px; height:28px; line-height:28px; flex-shrink:0; text-align:center;">${agent.emoji}</div>
            <div style="flex:1; min-width:0;">
              <div style="display:flex; align-items:center; gap:6px;">
                <span style="font-weight:bold; font-size:12px; color:#fff;">${name}</span>
                <span style="font-size:10px; background:#1b2d1d; color:#22c55e; padding:1px 5px; border-radius:3px;">${item.intervalHours}시간마다</span>
              </div>
              <div class="muted small" style="white-space:nowrap; overflow:hidden; text-overflow:ellipsis; max-width:240px;" title="${escapeHtml(item.task)}">${escapeHtml(item.task)}</div>
            </div>
          </div>
          <button class="auto-del-btn" data-id="${item.id}" style="background:transparent; border:none; color:#f43f5e; cursor:pointer; font-size:14px; padding:4px;" title="스케줄 정지">✕</button>
        </div>
      `;
    }).join('');
  ...
  // Wire up delete click handlers for main container
  container.querySelectorAll('.auto-del-btn').forEach(btn => {
    btn.addEventListener('click', async (e) => {
      const id = (e.target as HTMLElement).dataset.id;
      if (id) {
        await connect.automationStop(id);
        hint('🔄 자동화 스케줄이 정지되었습니다.');
        renderAutomations();
      }
    });
  });

  const dashContainer = $('dashAutoListContainer');
  if (dashContainer) {
    dashContainer.innerHTML = listHtml;
    dashContainer.querySelectorAll('.auto-del-btn').forEach(btn => {
      btn.addEventListener('click', async (e) => {
        const id = (e.target as HTMLElement).dataset.id;
        if (id) {
          await connect.automationStop(id);
...
function renderTeamRoster() {
  ...
  if (dashEl) {
    ...
    dashEl.querySelectorAll('.tr-card').forEach(c => c.addEventListener('click', () => openAgentDetail((c as HTMLElement).dataset.id!)));
  }
}
$('officePreview')?.addEventListener('click', () => { openOverlay('officePanel'); connect.officeOpen?.(); });
...
const DEF_PARAMS = { temp: 0.7, topP: 0.9, topK: 40, minP: 0.05, repeatPenalty: 1.1, freqPenalty: 0, presPenalty: 0, repeatLastN: 64, flashAttn: true, ctxSize: 8192, maxTokens: 1024, ngl: 32 };
...
async function renderTaskResults() {
  const container = $('taskResultContainer');
  if (!container) return;

  const list = await (connect as any).tasksResults?.().catch(() => []) || [];
  if (list.length === 0) {
    container.innerHTML = `<div class="muted small" style="text-align:center; padding:12px;">아직 완료된 작업 결과물이 없습니다.</div>`;
    return;
  }

  container.innerHTML = list.map((t: any) => {
    const isCompleted = t.status === 'completed';
    const borderCol = isCompleted ? '#22c55e' : '#ef4444';
    const statusText = isCompleted ? '✅ 완료' : '❌ 실패';
    const timeText = t.result.finishedAt ? new Date(t.result.finishedAt).toLocaleTimeString('ko-KR', { hour: '2-digit', minute: '2-digit' }) : '';
    
    return `
      <div class="result-card" style="border: 1px solid #111a2e; border-left: 4px solid ${borderCol}; border-radius: 4px; padding:8px 12px; background:#0c111c; display:flex; flex-direction:column; gap:6px;">
        <div style="display:flex; justify-content:space-between; align-items:center; font-size:12px;">
          <span style="font-weight:bold; color:#fff; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; max-width:70%;" title="${escapeHtml(t.title)}">${escapeHtml(t.title)}</span>
          <span style="font-size:10px; color:${isCompleted ? '#22c55e' : '#ef4444'}; font-weight:bold; flex-shrink:0;">${statusText} <span style="color:#555; font-weight:normal; margin-left:4px;">${timeText}</span></span>
        </div>
        <pre class="cmd-box" style="margin:0; padding:6px; background:#050810; border: 1px solid #0f172a; border-radius:3px; max-height:100px; overflow-y:auto; font-size:11px; font-family:var(--font-mono, monospace); color:#cbd5e1; white-space:pre-wrap; word-break:break-all;">${escapeHtml(t.result.output || '')}</pre>
      </div>
    `;
  }).join('');
}
...
const API_SERVICES: any[] = [
  ...
  { id: 'google-calendar', name: 'Google Calendar', icon: '📅', summary: '비서가 일정을 읽고 쓰고 task 마감일과 자동 동기화합니다. Cloud Console에서 OAuth 클라이언트를 만들고 ⚡ 자동 연결을 누르면 구글 로그인 한 번으로 완료.', wizard: true, helpUrl: 'https://console.cloud.google.com/', fields: [
    { key: 'GOOGLE_CALENDAR_CLIENT_ID', label: 'Client ID', type: 'password', help: 'Cloud Console → API 및 서비스 → 사용자 인증 정보 → OAuth 2.0 클라이언트 ID. 리디렉션 URI에 http://127.0.0.1:5815/calendar-callback 추가' },
    { key: 'GOOGLE_CALENDAR_CLIENT_SECRET', label: 'Client Secret', type: 'password' },
    { key: 'GOOGLE_CALENDAR_ID', label: 'Calendar ID (선택)', type: 'text', placeholder: 'primary', help: '비워두면 primary 사용. 특정 캘린더는 구글 캘린더 설정에서 확인' } ] },
  ...
  { id: 'huggingface', name: 'HuggingFace — 🧬 장기 기억', icon: '🤗', summary: '쌓인 지식을 데이터셋으로 업로드 → 모델에 파인튜닝(체득). 학습된 모델을 회사 뇌로 사용.', helpUrl: 'https://huggingface.co/settings/tokens', fields: [
    ...
    { key: 'HF_REPO', label: '데이터셋 이름', type: 'text', placeholder: 'axios-cli-brain', help: '이름만 적으면 돼요 (아이디는 토큰에서 자동). HF에서 미리 안 만들어도 자동 생성.' } ] },
  { id: 'email', name: '이메일 (Gmail SMTP+IMAP)', icon: '📧', summary: '받은 메일을 읽고(IMAP) AI가 답장 초안 → 텔레그램으로 보낼까요? 승인하면 발송(SMTP). Gmail 앱 비밀번호 하나로 둘 다.', helpUrl: 'https://support.google.com/accounts/answer/185833', fields: [
    ...
    { key: 'IMAP_HOST', label: '받기 IMAP 호스트(선택)', type: 'text', placeholder: 'imap.gmail.com (비우면 자동)' } ] },
        { id: 'instagram', name: 'Instagram (Meta Graph)', icon: '📷', summary: '인스타 비즈니스 게시 + DM/댓글 분석.', helpUrl: 'https://developers.facebook.com/', fields: [
  ...
  $('apiGrid').querySelectorAll('.svc-card').forEach(card => {
    ...
    card.querySelector('[data-act=wizard]')?.addEventListener('click', async () => {
      if (id === 'google-calendar') {
        hint('⚡ 브라우저에서 구글 로그인하세요… (Client ID/Secret 먼저 저장)');
        const r = await (connect as any).calendarOAuth?.();
        hint(r?.ok ? '✅ Google Calendar 연결 완료!' : `⚠️ ${r?.error || '연결 실패'}`);
...
const OFFICE_BG = '../../assets/map.jpeg';   // 사용자가 넣은 AXIOS CLI 대형 사무실 맵 (2912×1440, 2:1)
...
function startSpriteLoop() {
  ...
  if (voRaf) return;
  const tick = () => {
    if ($('officePanel').classList.contains('hidden')) {
      stopSpriteLoop();
      return;
    }
    voFrame++;
...
connect.onBridgeInject?.((d: any) => {
  ...
  playInjection(`${emoji[d.kind] || '🔌'} Axios AI Lab 지식 스토어 → ${d.kind === 'knowledge' ? '두뇌' : '작업실'}`, [d.label || '브레인팩 주입'], (CAT_META[d.category] || CAT_META.general).color);
...
$('hfExportBtn').addEventListener('click', async () => { $('hfStatus').textContent = '📦 바탕화면 axios-cli-brain.jsonl 확인 (변환 시 자동 저장)'; });
...
async function openAuth() {
  ...
  if (me?.email) {
    ...
    body.innerHTML = `<div class="auth-me"><div class="auth-ava">👤</div><div><div class="auth-email">${escapeHtml(me.email)}</div><div class="muted small">권한 등급: ${escapeHtml(me.role || 'Guest')} · 무료 회원 · 학습 월 1회</div></div></div><button class="cyc-btn ghost" id="authLogout" style="width:100%;margin-top:12px">로그아웃</button>`;
...
async function renderBrain() {
  const [g, list, count, stats] = await Promise.all([
    connect.brainGraph(),
    connect.brainList(),
    connect.brainCount(),
    (connect as any).brainStats?.() ?? Promise.resolve([]),
  ]);
  ...
  $('brainNotes').innerHTML = list.length
    ...
    : '<div class="muted" style="text-align:center;padding:14px">아직 지식이 없어요. ⬇ GitHub 불러오기, 🧠 Axios AI Lab 지식 스토어에서 주입, 또는 대화 중 에이전트가 자동으로 쌓아요.</div>';
...
async function renderBridge() {
  ...
  if (b.state === 'listening') { el.className = 'bridge-row on'; el.innerHTML = `🧠 Axios AI Lab 지식 스토어 <b>연결됨</b> (:${b.port}) — 스토어에서 [주입] 누르면 에이전트 두뇌로 들어와요`; }
  ...
  else { el.className = 'bridge-row'; el.innerHTML = `🧠 Axios AI Lab 지식 스토어 대기 중 (:${b.port})`; }
}
...
let automationsES: EventSource | null = null, automationsMap: Record<string, any> = {};

async function ensureAutomationsStream() {
  if (automationsES) return;
  const config = await connect.getConfig?.().catch(() => null) || {};
  const dbUrl = config.apiConn?.firebase?.FIREBASE_DB_URL || config.firebaseDbUrl;
  if (!dbUrl || !/^https?:\/\//.test(dbUrl)) return;
  const me = await connect.authCurrent?.().catch(() => null);
  const authSuffix = me?.idToken ? `?auth=${me.idToken}` : '';
  automationsES = subscribePath(dbUrl, 'schedulers', automationsMap, () => {
    renderAutomations();
  }, authSuffix);
}

let systemStatusES: EventSource | null = null;
let systemStatusMap: Record<string, any> = {};

async function ensureSystemStatusStream() {
  if (systemStatusES) return;
  const config = await connect.getConfig?.().catch(() => null) || {};
  const dbUrl = config.apiConn?.firebase?.FIREBASE_DB_URL || config.firebaseDbUrl;
  if (!dbUrl || !/^https?:\/\//.test(dbUrl)) return;
  const me = await connect.authCurrent?.().catch(() => null);
  const authSuffix = me?.idToken ? `?auth=${me.idToken}` : '';
  systemStatusES = subscribePath(dbUrl, 'system/status', systemStatusMap, () => {
    const badge = document.getElementById('agentActiveBadge');
    if (badge) {
      if (systemStatusMap.active) {
...
async function plazaToggleFn() {
  if (!plazaJoined) {
    ...
    if (r.uid) {
      ...
      plazaPeople[r.uid] = { uid: r.uid, company: myCompany, emoji: myEmoji, agents: [], source: 'axios-cli', ts: Date.now() };
      renderDesks();
    }
  } else {
...
function subscribePath(url: string, fullPath: string, store: Record<string, any>, onChange: () => void, authSuffix = ''): EventSource {
  const es = new EventSource(`${url.replace(/\/$/, '')}/${fullPath}.json${authSuffix}`);
...
function subscribe(url: string, sub: string, store: Record<string, any>, onChange: () => void, authSuffix = ''): EventSource {
  return subscribePath(url, `plaza/rooms/lobby/${sub}`, store, onChange, authSuffix);
}
async function ensurePlazaStream() {
  ...
  const me = await connect.authCurrent?.().catch(() => null);
  const authSuffix = me?.idToken ? `?auth=${me.idToken}` : '';
  plazaES = subscribe(url, 'messages', plazaMsgs, onMessages, authSuffix);
  plazaPresES = subscribe(url, 'presence', plazaPeople, renderDesks, authSuffix);
}
...
function pwEnsureLoop() {
  if (!plazaJoined || $('plazaPanel').classList.contains('hidden')) return;
  if (pwRaf) return;
  const PT = 48, PCH = 96;   // 스프라이트 셀 (사무실과 동일)
  const tick = () => {
    if (!plazaJoined || $('plazaPanel').classList.contains('hidden')) {
      pwStopLoop();
      return;
    }
    pwFrame++;
...
$('closePwDexBtn')?.addEventListener('click', () => { $('pwDex')?.classList.add('hidden'); });
...
connect.onEngineEvent?.((e: any) => {
  if (e.type === 'task:added' || e.type === 'task:done' || e.type === 'task:updated') {
    renderTasks();
    renderTaskResults();
  }
  if (e.type === 'approval:added' || e.type === 'approval:approved') {
    renderApprovals();
  }
});
export {};
