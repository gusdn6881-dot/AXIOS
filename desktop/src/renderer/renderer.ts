...
const API_SERVICES: any[] = [
  ...
  { id: 'huggingface', name: 'HuggingFace — 🧬 장기 기억', icon: '🤗', summary: '쌓인 지식을 데이터셋으로 업로드 → 모델에 파인튜닝(체득). 학습된 모델을 회사 뇌로 사용.', helpUrl: 'https://huggingface.co/settings/tokens', fields: [
    ...
    { key: 'HF_REPO', label: '데이터셋 레포', type: 'text', placeholder: 'user/axios-ai-brain' } ] },
...
const OFFICE_BG = '../../assets/map.jpeg';   // 사용자가 넣은 Axios AI 대형 사무실 맵 (2912×1440, 2:1)
...
$('hfExportBtn').addEventListener('click', async () => {
  ...
  $('hfStatus').textContent = r.ok ? `✅ ${r.count}개 → 바탕화면/axios-ai-knowledge.jsonl` : `⚠️ ${r.reason}`;
});
