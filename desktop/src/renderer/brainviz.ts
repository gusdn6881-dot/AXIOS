...
export class BrainViz {
  ...
  private loop = () => {
    ...
    // Batch edge rendering: group edges into depth layers to reduce draw calls
    const numLayers = 4;
    const paths: Path2D[] = Array.from({ length: numLayers }, () => new Path2D());
    for (const [i, j] of this.edges) {
      const a = proj[i], b = proj[j];
      const depth = (a.z + b.z) / 2;
      const front = (depth + 1) / 2;
      const layerIdx = Math.max(0, Math.min(numLayers - 1, Math.floor(front * numLayers)));
      paths[layerIdx].moveTo(a.sx, a.sy);
      paths[layerIdx].lineTo(b.sx, b.sy);
    }
    
    for (let layerIdx = 0; layerIdx < numLayers; layerIdx++) {
      const front = (layerIdx + 0.5) / numLayers;
      const glow = this.energy * 0.8;
    ...
    for (let k = this.pulses.length - 1; k >= 0; k--) {
      ...
      // Draw glow halo (replaces expensive shadowBlur)
      ctx.fillStyle = `rgba(${pu.c[0]},${pu.c[1]},${pu.c[2]},${al * 0.25})`;
    ...
    for (const i of order) {
      ...
      // Draw glow halo (replaces expensive shadowBlur)
      const glowRad = rad * (1.8 + front * 1.5 + fire * 2.0);
      ctx.fillStyle = `rgba(0, 255, 153, ${bright * 0.22})`;
      ctx.beginPath(); ctx.arc(p.sx, p.sy, glowRad, 0, Math.PI * 2); ctx.fill();
