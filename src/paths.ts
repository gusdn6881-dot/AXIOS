/* v2.89.66 — 경로 유틸리티 모듈.
 *
 * 두뇌 폴더(`~/.axios-ai-brain/`) 와 회사 폴더(`<brain>/_company/` 또는 detached path) 의
 * 위치를 결정하는 함수들. 분리 이유:
...
/** Settings.json `axiosAi.localBrainPath` 입력 처리. ~/ 와 빈 문자열 케이스 정규화. */
...
/** 두뇌 폴더 위치 결정. settings.json `localBrainPath` 우선, 없으면 `~/.axios-ai-brain/`. */
export function _getBrainDir(): string {
    ...
    return path.join(os.homedir(), '.axios-ai-brain');
}
...
export function getCompanyDir(): string {
    try {
        const raw = vscode.workspace.getConfiguration('axiosAi').get<string>('companyDir', '') || '';
        const resolved = _resolvePathInput(raw);
        if (resolved) return resolved;
    } catch { /* config unavailable in some hot paths — fall through */ }
